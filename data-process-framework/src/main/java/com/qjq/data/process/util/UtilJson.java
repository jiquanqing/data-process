...
public class UtilJson {
	...
	public static <T> T readValue(String content) {
		if (UtilString.isEmpty(content)) return null;
		
		char ch = content.charAt(0);
		try {
			if (ch == '[') return (T) JSON_MAPPER.readValue(content, List.class);
	...
	public static <T> T convertValue(Object value, Class<T> clazz) throws IllegalArgumentException {
		if (UtilString.isEmpty(value)) return null;
		try {
			if (value instanceof String) value = JSON_MAPPER.readTree((String) value);
