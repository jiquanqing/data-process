...
public class DiamondPropertySourcesPlaceholderConfigurer extends PropertySourcesPlaceholderConfigurer {
    ...
    private PropertySourcesPropertyResolver buildResolver() {
        ...
        if (environment != null) {
            propertySources.addLast(new PropertySource<Environment>(ENVIRONMENT_PROPERTIES_PROPERTY_SOURCE_NAME, environment) {
                @Override
        ...
        if (diamondLocation != null) { // 存在diamond，初始化diamond client
            { // 先去掉locations中的diamond
                ...
                for (Resource location : locations) {
                    if (location != diamondLocation)
                        locationsExcludeDiamond[pos++] = location;
                }
            ...
            if (appId == null || appId < 1)
                throw new IllegalArgumentException("配置错误 server.appId=" + appId);
            ...
            DiamondClient client = new DiamondClient();
            {
                ...
                if (tmp != null && tmp >= 0)
                    client.setConnectionTimeout(tmp);
                tmp = resolver.getProperty("diamond.zk.sessionTimeout", Integer.class);
                if (tmp != null && tmp >= 0)
                    client.setSessionTimeout(tmp);
            }
            client.init();
            beanFactory.registerSingleton("DiamondClientZk", client);
            if (beanFactory instanceof DefaultListableBeanFactory)
                ((DefaultListableBeanFactory) beanFactory).registerDisposableBean("DiamondClientZk", client);

            // 重写diamond resource
            locations[diamondPos] = new ByteArrayResource(client.getAppPropertiesContent(appId).getBytes(), "diamond: "
